...
def get_accuracy_and_loss(loader, model, criterion):
    ...
    with torch.no_grad():
        for img, target in loader:
            ...
            predicted = output.round()  # Get the predictions
...
def train(
    data_with_board_root_dir="resources/chessboards_bbox_images/chessboards_bbox",
    data_no_board_root_dir="resources/chessboards_bbox_images/no_chessboards",
    outdir="models",
    total_steps=10_000,
    batch_size=8,
    max_lr=0.001,
    ...
    board_image_set = dataset.ExistenceDataset(
        with_board_root_dir=data_with_board_root_dir,
        no_board_root_dir=data_no_board_root_dir,
        augment_ratio=0.8,
    ...
    file_name = outdir + "/best_model_existence_%.3f_%s.pth" % (
        best_acc,
        start_time_string,
    )
